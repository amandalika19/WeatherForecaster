...
function App() {
    ...
    const handleSubmit = async (e) => {
        ...
        const weatherForecast = periods ? periods.filter(period => {
            const selectedDate = date.split('T')[0];
            let periodDate = period.startTime.split('T')[0];

            if (timePeriod === 'Night') {
                return periodDate === selectedDate && (!period.isDaytime || period.name === 'Tonight');
            } else {
                return periodDate === selectedDate && (period.isDaytime || period.name === 'This Afternoon');
            }
        }) : [];
